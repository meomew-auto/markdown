...
Một user loop tạo nhiều cart operations. RPS tổng có thể cao hơn iter/s nhiều lần.

### Nguyên nhân kỹ thuật 2: Summary read can hide behind write success

Add/update 200 chưa chứng minh read model/summary đúng hoặc nhanh.

### Nguyên nhân kỹ thuật 3: Same active user mutates state over time

Constant-vus giữ active user identity qua duration, phù hợp cart interaction lặp lại.

### Nguyên nhân kỹ thuật 4: Backend slowdown reduces loop rate

Nếu writes lock DB, VUs vẫn flat nhưng loops/s giảm thay vì k6 ép thêm arrivals.
